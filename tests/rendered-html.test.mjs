...
test("ships the complete saved-content indexes without private repositories", async () => {
  ...
  assert.equal(xCount, 2714);
  assert.equal(githubStars.length, 3349);
  assert.equal(linkedInCount + xCount + githubStars.length, 6334);
  ...
  assert.match(githubMeta, /["']?count["']?: 3349/);
