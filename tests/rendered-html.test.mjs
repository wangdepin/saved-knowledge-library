...
test("ships the complete saved-content indexes without private repositories", async () => {
  ...
  assert.equal(linkedInCount, 265);
  assert.equal(xCount, 2547);
  assert.equal(githubStars.length, 3322);
  assert.equal(linkedInCount + xCount + githubStars.length, 6134);
  ...
  assert.match(githubMeta, /["']?count["']?: 3322/);
