...
test("ships the complete saved-content indexes without private repositories", async () => {
  ...
  assert.equal(linkedInCount, 275);
  assert.equal(xCount, 2760);
  assert.equal(githubStars.length, 3355);
  assert.equal(linkedInCount + xCount + githubStars.length, 6390);
  ...
  assert.match(githubMeta, /["']?count["']?: 3355/);
