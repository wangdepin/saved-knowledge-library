...
test("ships the complete saved-content indexes without private repositories", async () => {
  ...
  assert.equal(githubStars.length, 3335);
  assert.equal(linkedInCount + xCount + githubStars.length, 6207);
  ...
  assert.match(githubMeta, /["']?count["']?: 3335/);
