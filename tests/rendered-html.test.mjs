...
test("ships the complete saved-content indexes without private repositories", async () => {
  ...
  assert.equal(linkedInCount, 274);
  assert.equal(xCount, 2737);
  assert.equal(githubStars.length, 3352);
  assert.equal(linkedInCount + xCount + githubStars.length, 6363);
  ...
  assert.match(githubMeta, /["']?count["']?: 3352/);
