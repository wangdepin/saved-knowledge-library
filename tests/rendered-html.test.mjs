...
test("ships the complete saved-content indexes without private repositories", async () => {
  ...
  assert.equal(xCount, 2606);
  assert.equal(githubStars.length, 3330);
  assert.equal(linkedInCount + xCount + githubStars.length, 6202);
  ...
  assert.match(githubMeta, /["']?count["']?: 3330/);
