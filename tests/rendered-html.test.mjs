...
test("ships the complete saved-content indexes without private repositories", async () => {
  ...
  assert.equal(linkedInCount, 220);
  assert.equal(xCount, 2403);
  assert.equal(githubStars.length, 3266);
  assert.equal(linkedInCount + xCount + githubStars.length, 5889);
  ...
  assert.match(githubMeta, /["']?count["']?: 3266/);
